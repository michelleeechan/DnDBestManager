...
- 貨幣 chip 可拖到另一個容器 → prompt 問搬移數量（預設全部，上限為源容器現有量）。點金額可直接輸入新值或 `+50` / `-50` 算式。-/+ 按鈕短按＝加/減 1；長按（pointerdown 起 500ms 計時器）＝彈 prompt 輸入加減量。
...
4. 每張卡下方五個貨幣 chip：點數字輸入新值或 `+50`/`-50`；-/+ 按鈕**短按＝加/減 1**、**長按（約半秒）＝彈窗輸入加減量**；**拖 chip 到另一容器搬錢**（會問數量）。
...
## 八、v2 大改版（2026-09-06）：負重＋紙人穿備系統

本次由另一個 AI session 參考 https://cros.land/dnd-5e-inventory-manager/ 的功能設計（未抄其 UI／未加 SRD 預設物品，使用者明確不要預設 items），新增以下功能：

### 新功能一覽

1. **負重系統**：角色有 `str`（力量）欄位，負重上限＝力量×15 lb；5e 變體負重規則兩級警示（≥5×力量＝輕度負重速度-10呎、≥10×力量＝重度負重速度-20呎、>15×力量＝超載紅字）。角色頭部有負重進度條（33.3%/66.6% 有門檻刻度線）。「金幣計重」開關（50 枚＝1 lb，計入負重）。
2. **紙人穿備系統（ Equip Doll，含使用者自訂規格）**：
   - 十個裝備位：頭部/頸部/身體/主手/副手/戒指一/戒指二（放物品）＋背部/腰間一/腰間二（放**容器**）。
   - **容器（袋）可以 equip 上紙人**：腰包等容器可拖到紙人槽位或按「裝備」自動找空位。裝備中的袋子在紙人上顯示名稱、件數、內容重量，可展開/收合（▼/▲）。
   - **展開的物品清單在畫面右側**（#bagPanel），不跟參考站相同設計。清單中的物品可**直接拖到其他容器卡或紙人上的其他袋子**搬移。
   - **丟棄／撿起**：物品列有「丟棄」鈕→進入右下「丟棄區（腳邊）」；丟棄區每件獨立有「撿起」（還原到原容器，原容器不在則進目前展開的容器）與「永久刪除」。整個袋子也可丟棄（連同內容物與金錢打包進丟棄區，撿起整包還原）。
   - **數量加減照金錢的設計**：短按 +/- 加減 1、長按 500ms 彈 prompt 輸入算式、點數字直接輸入新值或 +50/-50。重量隨數量即時變化。
   - 容器 equip/unequip 即時更新角色負重（只有「裝備中的容器內容＋裝備中的物品」計入負重；未裝備容器的東西不算在身上）。
3. **同調系統**：物品可勾「需同調」，再切換同調狀態；同調上限 3 件，超過會 alert 擋下；角色頭部顯示「同調 n/3」。
4. **物品欄位擴充（左側詳情面板）**：點物品名稱→左面板顯示詳情：名稱/數量/單件重量/單件價值(gp)/分類(其他/武器/防具/工具/消耗品/魔法物品)/備註/需同調，另有裝備、移到容器、丟棄、刪除。物品價值計入「總值」。
5. **排序與檢視**：右側面板有排序（數量/重量/價值/名稱）與 列表/格子 兩種檢視。
6. **容器容量**：每個容器可設容量上限（lb，0=不限），容器卡與右側面板都有進度條，≥80% 變黃、超過變紅+⚠。
7. 物品列的 ✎ 或點名稱＝選取（左面板編輯）；舊的 `editItemDialog`（prompt 連環問）已移除。

### 資料模型 v2（data.json / localStorage key 仍為 dnd-inv-v1）

```json
"characters": [{
  "id","name",
  "str": 10, "coinWeight": false,
  "containers": [{ "id","name","money":{...}, "cap": 0, "equipped": false, "slot": null }],
  "items": [{ "id","cid","name","qty","weight","notes",
              "value":"", "type":"misc", "attune":false, "attuned":false, "equipped":false, "slot":null }],
  "dropped": [{ "id", "kind":"item", "item":{...} | "kind":"cont","cont":{...},"items":[...] }]
}]
```

`migrate()` 在 load/pull/import 時自動把 v1 資料補上預設欄位（str=10、所有容器/物品 equipped=false 等），舊資料無痛升級。版本號 version: 2。

### 版面

`.ws` 三欄 grid：左=物品詳情面板(#leftPanel)｜中=紙人(.dollzone：SVG 小人+絕對定位槽位)+未裝備容器卡｜右=容器內容(#bagPanel)+丟棄區。≤1150px 摺疊為單欄（順序：紙人→右面板→左面板）。

### 本次踩坑／備註

- **坑 7：物品價值單位**：`totalGp()` 內全以 cp 計算，item.value 是 gp，第一次寫忘了 ×RATE.gp，總值會小 100 倍。已修（上傳前煙霧測試抓到）。
- **坑 8：i18n key 拼法**：狀態字串 key 用 `enc_ok/enc_light/enc_heavy/enc_over`（底線式），與 CSS class（.encst.ok 等，無底線）**不同**，改 key 時兩邊要分開對照。`capUnlimited` 一度漏定義，畫面直接印出 raw key。教訓：渲染字串一律經 `t()`，測試時要 grep 畫面有無 raw key。
- 自動化測試環境（in-app browser）不支援 `prompt()` 對話框，prompt 流程無法自動化測（v1 已在真瀏覽器驗證過同機制）；其餘用 localStorage 注入＋合成 DragEvent 完成煙霧測試：負重計算、裝備/卸下/重裝、拖放搬移（item→容器卡）、丟棄/撿起、排序、格子檢視、v1 遷移、中英切換全部通過。
- 本次上傳流程沿用第零節規則（Write 工具中轉＋node base64＋curl PUT＋重新下載 cmp 逐位元組比對＋中文抽查），app.js 41129 bytes、style.css 11002 bytes，皆驗證一致。
- 上傳後本機臨時檔與 clone 已全部刪除（使用者要求一切在 GitHub）。

### 已知限制（v2 新增）

- 拖放是 HTML5 DnD，手機觸控不支援拖（手機仍可用「移到容器」下拉搬移）。
- 容器本身沒有自重欄位（袋子的皮重）。
- 裝備位沒有限制物品類型（任何物品都可裝到任何物品槽）；寶物清單貼上解析（參考站的 loot splitter）尚未實作。
